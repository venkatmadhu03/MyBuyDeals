package appsnova.com.mybuydeals.ownlibraries;
...
/**
 * A Material style progress Dialog.
 * Created by Maddy on 19/01/2019.
 * at AppsNova Software Solutions
 *
 */
public class MaterialProgressWheel extends View {
    ...
    private double spinCycleTime = 460;
     ...
     * @param attrs
     */
    public MaterialProgressWheel(Context context, AttributeSet attrs) {
        super(context, attrs);
     ...
     * @param context
     */
    public MaterialProgressWheel(Context context) {
        super(context);
    }
    ...
    private void parseAttributes(TypedArray a) {
        ...
        circleRadius = (int) a.getDimension(R.styleable.ProgressWheel_mbdProg_circleRadius, circleRadius);

        fillRadius = a.getBoolean(R.styleable.ProgressWheel_mbdProg_fillRadius, false);

        barWidth = (int) a.getDimension(R.styleable.ProgressWheel_mbdProg_barWidth, barWidth);

        rimWidth = (int) a.getDimension(R.styleable.ProgressWheel_mbdProg_rimWidth, rimWidth);

        float baseSpinSpeed = a.getFloat(R.styleable.ProgressWheel_mbdProg_spinSpeed, spinSpeed / 360.0f);
        spinSpeed = baseSpinSpeed * 360;

        spinCycleTime = a.getInt(R.styleable.ProgressWheel_mbdProg_spinCycleTime, (int) spinCycleTime);

        barColor = a.getColor(R.styleable.ProgressWheel_mbdProg_barColor, barColor);


        rimColor = a.getColor(R.styleable.ProgressWheel_mbdProg_rimColor, rimColor);

        linearProgress = a.getBoolean(R.styleable.ProgressWheel_mbdProg_linearProgress, false);

        if (a.getBoolean(R.styleable.ProgressWheel_mbd_progressIndeterminate, false)) {
            spin();
        }
    ...
    private void updateBarLength(long deltaTimeInMilliSeconds) {
        if (pausedTimeWithoutGrowing >= pauseGrowingTime) {
            ...
            if (timeStartGrowing > spinCycleTime) {
                // We completed a size change cycle
                // (growing or shrinking)
                timeStartGrowing -= spinCycleTime;
            ...
            float distance = (float) Math.cos((timeStartGrowing / spinCycleTime + 1) * Math.PI) / 2 + 0.5f;
